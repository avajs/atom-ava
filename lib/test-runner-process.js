...
import AssertResultFactory from './assert-result-factory';

class TestRunnerProcess extends EventEmitter {
	constructor(
		executor = new TerminalCommandExecutor(),
		parserFactory = new ParserFactory(),
		assertResultFactory = new AssertResultFactory()) {
		super();

		this.eventHandlers = {};
		this.parserFactory = parserFactory;
		this.terminalCommandExecutor = executor;
		this.assertResultFactory = assertResultFactory;
	...
	runAll(folder) {
		...
		this._runCommand('ava', ['--tap'], folder, context);
	}
	...
	_setHandlersOnParser(parser) {
		...
		parser.on('assert', assert => {
			...
			const result = this.assertResultFactory.getAssertResult(assert, this.currentExecutionContext);
	...
	_updateCurrentExecution(assert) {
		if (assert.ok) {
			if (!assert.skip) {
				this.currentExecutionContext.passed++;
			}
		} else if (!assert.todo) {
			this.currentExecutionContext.failed++;
		}
	}
